...
You MUST create a todo (via SQL) for each of these items and complete them in order:

1. **Explore project context** — use `view` and `grep` to check files, docs, recent commits
...
5. **Write design doc** — save to `docs/specs/YYYY-MM-DD-<topic>-design.md` and commit (or user's preferred location)
...
- Check out the current project state first (use `view`, `grep`, `glob` to explore files, docs, recent commits)
...
- Ask questions one at a time using the `ask_user` tool to refine the idea
- Prefer multiple choice questions when possible, but open-ended is fine too
- Only one question per message — break multi-part topics into separate questions
...
- Smaller, well-bounded units are easier to work with — reasoning is more reliable when files are focused

**Working in existing codebases:**

- Use `grep` and `glob` to explore the current structure before proposing changes. Follow existing patterns.
...
- **Multiple choice preferred** — Use `ask_user` with `choices` when possible
